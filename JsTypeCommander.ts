...
let whitespaceRegex: RegExp = /^\s*$/m;
let trimStartRegex: RegExp = /^\s+(\S.*)?$/m;
let trimEndRegex: RegExp = /^(\s*\S+(\s+\S+)*)/m;
...
export function isEmptyOrWhitespace(obj?: TDefined): obj is string {
    return mapByTypeValue<any, boolean>(obj, {
        whenString: (s) => whitespaceRegex.test(s),
        otherwise: false
    });
}
...
export function indentText(text: string|string[], indent?: string): string {
    ...
    if (i.length == 0)
        i = "\t";
    let t = (Array.isArray(text)) ? text.join(newLineString) : toString(text, "");
 ...
 */
export function toArray(obj?: TDefined, simpleCheck?: boolean): TAnythingAtAll[] {
    if (isArray(obj))
        return obj;
